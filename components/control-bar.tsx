...
import { Mic, MicOff, Video, VideoOff, Phone, PhoneOff, Image, Share, MoreHorizontal } from "lucide-react"
    ...
    return (
        <div className="flex items-center justify-center space-x-3 bg-transparent">
            ...
            <button
                onClick={toggleAudio}
                disabled={isLoading}
                className={`p-3 rounded-full ${isAudioMuted ? "bg-gray-200 text-gray-700" : "bg-gray-100 text-gray-700"
            ...
            <button
                onClick={toggleVideo}
                disabled={isLoading}
                className={`p-3 rounded-full ${!isVideoEnabled ? "bg-gray-200 text-gray-700" : "bg-gray-100 text-gray-700"
                    } hover:bg-gray-200 transition-colors`}
            ...
            {!remoteStream ? (
                <button
                    ...
                    className={`p-3 rounded-full ${isLoading || !isVideoEnabled || callStatus !== "idle"
                ...
                <button
                    ...
                    className={`p-3 rounded-full ${isLoading || callStatus === "ending"
                        ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                    ...
                    <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-white rounded-lg shadow-lg p-3 w-64 z-10">
                        ...
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button
                                ...
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${backgroundRemovalEnabled ? "bg-blue-600" : "bg-gray-200"
                                    }`}
                            >
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${backgroundRemovalEnabled ? "translate-x-6" : "translate-x-1"
                                        }`}
                                />
                            </button>
                        </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {backgroundOptions.map((bg) => (
                                        <button
                                            ...
                                            className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all ${selectedBackground === bg.id
            ...
            <button
                ...
                className={`p-3 rounded-full ${isLoading || callStatus !== "connected"
                    ? "bg-gray-200 text-gray-500 cursor-not-allowed"
